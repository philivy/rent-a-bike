  ...
  try {
    ...
    if (result.rows.length > 0) {
      const articles = result.rows.map(article => ({
        ...article,
        photo: article.photo ? `data:image/jpeg;base64,${article.photo.toString('base64')}` : null,
        qrcode: article.qrcode ? `data:image/png;base64,${article.qrcode.toString('base64')}` : null
      }));
    ...
    console.error("❌ Erreur SQL lors de la recherche d'articles :", error.stack);
